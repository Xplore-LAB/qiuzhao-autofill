...
const browser=['extension-lifecycle-regression.cjs','phoenix-close-browser-regression.cjs','choice-close-browser-regression.cjs','zhuanzhuan-extension-regression.cjs','quick-ui-regression.cjs','sidepanel-regression.cjs'];
...
const inputs=[...runtime,'package.json','scripts/check-extension.cjs','scripts/triage-run-logs.cjs',...unit.concat(browser).map(f=>'test/'+f),'test/popup-mock-chrome.js'].sort();
...
function run(label,file,args=[]){
 const result=cp.spawnSync(process.execPath,[...args,path.join(root,file)],{cwd:root,env:process.env,encoding:'utf8',timeout:120000,maxBuffer:4*1024*1024,windowsHide:true});
