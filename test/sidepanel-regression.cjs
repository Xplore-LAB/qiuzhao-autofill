...
(async()=>{
 ...
 try{
  ...
  await page.addInitScript({content:`window.active=7;window.sent=[];window.chrome={storage:{local:{get:async()=>({profile:{name:'演示姓名'},settings:{}})}},runtime:{getManifest:()=>({version:'1.16.6'}),sendMessage:async()=>({ok:true,frames:[{frameId:0,host:'fixture.invalid',totalControls:3}]})},tabs:{query:async()=>[{id:active}],sendMessage:async(id,m)=>{sent.push({id,type:m.type});if(m.type==='PREVIEW_FORM')return {previewToken:'demo-preview',totalControls:3,filledControls:0,ruleCandidates:2,aiCandidates:1};return m.type==='PING'?{host:'fixture.invalid',contentBuild:'1.16.6-dev'}:{running:false};}}};`});
