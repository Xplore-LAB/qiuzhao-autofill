...
async function rangeTransaction(){
 ...
 w.eval('let open=false,closes=0;function visibleChoiceLayers(){return open?[document.querySelector("#layer")]:[];}function ownedChoiceLayer(a,l){return l[0]||null;}async function dismissVisibleChoiceLayers(){closes++;open=false;const inputs=document.querySelectorAll("input");if(!inputs[1].value)inputs[0].value="";return true;}function safeCustomClick(e){e.click();open=true;return true;}async function wait(){}async function waitForControlState(p){return p();}async function fillAntCalendar(a,l,m){a.querySelector("input").value=m[1]+"-"+m[2]+"-"+m[3];return true;}'+cut('  const openDateRanges =','  async function fillAntCalendar(')+'window.fill=fillCustomDate;window.count=()=>closes;');
 try{
